...
public protocol BaasLoginStrategy {
    ...
    func login(_ username: String, password: String, completion: BaasBox.BaasCompletion)
    /// override for social login, you can use the default one like this
    ///     BaasDefaultLoginStrategy().socialLogin(...)
    func socialLogin(_ to: BaasBox.socialTypes, token: String, completion: BaasBox.BaasCompletion)
}
...
public protocol BaasSignupStrategy {
    ...
    func signup(_ username: String, password: String, visibleByTheUser : [String: Any], visibleByFriends : [String: Any], visibleByRegisteredUsers : [String: Any], visibleByAnonymousUsers : [String: Any], completion: BaasBox.BaasCompletion)
}
/**
...
class BaasDefaultLoginStrategy: BaasLoginStrategy {
    func login(_ username: String, password: String, completion: BaasBox.BaasCompletion) {
        ...
        BaasBox.shared.post("/login", parameters: ["username": username, "password":password, "appcode": BaasBox.getAppcode()!], completion: { (success, data, error) in
            if success {
                print(data!)
                if BaasBox.getConfig().retrylogin { BaasBox.saveUserPass(password) }
    ...
    func socialLogin(_ to: BaasBox.socialTypes, token: String, completion: BaasBox.BaasCompletion) {
        ...
        BaasBox.shared.post("/social/"+to.rawValue, parameters: ["oauth_token": token, "oauth_secret": token], completion: { (success, data, error) in
            if success {
                if BaasBox.getConfig().retrylogin { BaasBox.saveUserPass(token) }
...
class BaasDefaultSignupStrategy : BaasSignupStrategy {
    func signup(_ username: String, password: String, visibleByTheUser: [String : Any], visibleByFriends: [String : Any], visibleByRegisteredUsers: [String : Any], visibleByAnonymousUsers: [String : Any], completion: BaasBox.BaasCompletion) {
        ...
        BaasBox.shared.post("/user", parameters: ["username" : username, "password" : password, "visibleByTheUser" : visibleByTheUser, "visibleByFriends" : visibleByFriends, "visibleByRegisteredUsers" : visibleByRegisteredUsers, "visibleByAnonymousUsers" : visibleByAnonymousUsers], completion: {

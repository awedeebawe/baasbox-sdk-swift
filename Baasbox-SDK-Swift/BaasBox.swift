...
open class BaasBox {
    ...
    fileprivate var appcode : String?
    fileprivate var config : BaasConfig!
    
    fileprivate var version = "v1.0"
     ...
     */
    open static func setup(_ url: String, appcode: String, config: BaasConfig = BaasConfig()) {
    ...
    */
    open static func getAppcode() -> String? {
     ...
     */
    open static func getBaseUrl() -> String? {
     ...
     */
    open static func getConfig() -> BaasConfig {
     ...
     */
    open func login(_ username: String, password: String, completion: BaasCompletion ) {
        check()
        self.config.loginStrategy.login(username, password: password, completion: {(success, data, error) in
     ...
     */
    open func socialLogin(_ to: socialTypes, token: String, completion: BaasCompletion ) {
        self.config.loginStrategy.socialLogin(to, token: token, completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func createUser(_ username: String, password: String, visibleByTheUser : [String: Any] = [:], visibleByFriends : [String: Any] = [:], visibleByRegisteredUsers : [String: Any] = [:], visibleByAnonymousUsers : [String: Any] = [:], completion: BaasCompletion ) {
        self.config.signupStrategy.signup(username, password: password, visibleByTheUser: visibleByTheUser, visibleByFriends: visibleByFriends, visibleByRegisteredUsers: visibleByRegisteredUsers, visibleByAnonymousUsers: visibleByAnonymousUsers, completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    /**
     ...
     */
    open func getUser(_ username: String, completion: BaasCompletion ) {
        assert(username != "", "Username of user to fetch cannot be empty")
        get("/user/"+username, parameters: [:], completion: { (success, data, error) in
            completion(success, data, error)
        })
    }
    /**
     ...
     */
    open func getUserList(_ parameters : [String:AnyHashable] = [:], completion: BaasCompletion) {
        get("/users", parameters: parameters, completion: { (success, data, error) in
            completion(success, data, error)
        })
    }
    /**
     ...
     */
    open func resetPassword(_ username: String, completion: BaasCompletion) {
        assert(username != "", "Username of user to try reset password cannot be empty")
        get("/user/"+username+"/password/reset", parameters: [:], completion: { (success, data, error) in
            completion(success, data, error)
        })
    }
    
    fileprivate static func getSavedUser() -> BaasUser {
     ...
     */
    open static func saveCurrentUser() {
     ...
     */
    open static func deleteCurrentUser() {
     ...
     */
    open static func saveUserPass(_ pass: String) {
    ...
    fileprivate static func getUserPass() -> String? {
        let key = UIDevice.current.identifierForVendor!.uuidString
    ...
    /**
     ...
     - parameters:
        - username: the username to fetch
     */
    open func getFollows(_ username: String, completion: BaasCompletion) {
        assert(username != "", "Username cannot be empty")
        get("/following/"+username, parameters: [:], completion: { (success, data, error) in
            completion(success, data, error)
        })
    }
    /**
     ...
     - parameters:
        - username: the username to fetch
     */
    open func getFollowers(_ username: String, completion: BaasCompletion) {
        assert(username != "", "Username cannot be empty")
        get("/followers/"+username, parameters: [:], completion: { (success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func createDocument(inCollection coll:String, body: [String: Any], completion: BaasCompletion){
        assert(coll != "", "Collection name cannot be empty")
        post("/document/"+coll, parameters: body, completion: { ( success, data, error ) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func deleteDocument(fromCollection coll: String, id: String, completion:BaasCompletion) {
        assert(coll != "" && id != "", "Collection name and id of document cannot be empty")
        delete("/document/"+coll+"/"+id, parameters: [:], completion: { (success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func getDocuments(fromCollection coll: String, withID id: String = "", parameters: [String: AnyHashable] = [:], completion: BaasCompletion) {
        assert(coll != "", "Collection name cannot be empty")
        get("/document/"+coll+(id != "" ? "/"+id : ""), parameters: parameters, completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - parameters:
        ...
        - parameters: (Optional) query criteria
     */
    open func countDocuments(inCollection coll: String, parameters: [String:AnyHashable] = [:], completion: BaasCompletion) {
        assert(coll != "", "Collection name cannot be empty")
        get("/document/"+coll+"/count", parameters: parameters, completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    /**
     ...
     */
    open func updateDocument(inCollection coll: String, withID id: String, body: [String: Any], completion: BaasCompletion) {
        assert(coll != "", "Collection name cannot be empty")
        put("/document/"+coll+"/"+id, parameters: body, completion: {(success, data, error) in
            completion(success,data,error)
        })
    }
     ...
     */
    open func grantUserAccess(toDocument id: String, inCollection coll: String, toUser user: String, accessType: docAccessTypes, completion: BaasCompletion) {
        assert(coll != "" && id != "" && user != "", "Collection name, id of document and username cannot be empty")
        put("/document/"+coll+"/"+id+"/"+accessType.rawValue+"/user/"+user, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - parameters:
        ...
        - accessType: enumeration of types of access ( read, delete, all, update )
     */
    open func grantRoleAccess(toDocument id: String, inCollection coll: String, toRole roleid: String, accessType: docAccessTypes, completion: BaasCompletion) {
        assert(coll != "" && id != "" && roleid != "", "Collection name, id of document and role cannot be empty")
        put("/document/"+coll+"/"+id+"/"+accessType.rawValue+"/role/"+roleid, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - parameters:
        ...
        - accessType: enumeration of types of access ( read, delete, all, update )
     */
    open func revokeUserAccess(toDocument id: String, inCollection coll: String, toUser user: String, accessType: docAccessTypes, completion: BaasCompletion) {
        assert(coll != "" && id != "" && user != "", "Collection name, id of document and username cannot be empty")
        delete("/document/"+coll+"/"+id+"/"+accessType.rawValue+"/user/"+user, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - accessType: enumeration of types of access ( read, delete, all, update )
     */
    open func revokeRoleAccess(toDocument id: String, inCollection coll: String, toRole roleid: String, accessType: docAccessTypes, completion: BaasCompletion) {
        assert(coll != "" && id != "" && roleid != "", "Collection name, id of document and role cannot be empty")
        delete("/document/"+coll+"/"+id+"/"+accessType.rawValue+"/role/"+roleid, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func createLink(_ label: String, sourceID source: String, destinationID dest: String, completion: BaasCompletion) {
        assert(label != "" && source != "" && dest != "", "Label, source and destination for link cannot be empty")
        post("/link/"+source+"/"+label+"/"+dest, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - parameters:
        ...
        - parameters: (Optional) query criteria
     */
    open func getLinks(withID id: String = "", parameters: [String:AnyHashable] = [:], completion: BaasCompletion) {
        get("/link"+(id != "" ? "/"+id : ""), parameters: parameters, completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func deleteLink(withID id: String, completion: BaasCompletion) {
        assert(id != "", "Link id for deletion cannot be empty")
        delete("/link/"+id, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    */
    open func uploadFile(_ data: Data, attachedData: [String:AnyHashable] = [:], permissions: [String:AnyHashable] = [:], completion: BaasCompletion) {
        postImage("/file", image:data, attachedData: attachedData, permissions:permissions, completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func deleteFile(withID id: String, completion: BaasCompletion) {
        assert(id != "", "File id for deletion cannot be empty")
        delete("/file/"+id, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func getFile(withID id: String, completion: BaasFileCompletion) {
        ...
        getFile("/file/"+id, completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func getFileDetails(withID id: String, completion: BaasCompletion) {
        assert(id != "", "File id for details cannot be empty")
        guard id != "" else {
            completion(false, nil, NSError.init(domain: domain, code: -1, userInfo: ["error":"Id of file not provided"]))
            return
        }
        get("/file/details/"+id, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func getFilesDetails(_ parameters: [String: AnyHashable] = [:], completion: BaasCompletion) {
        get("/file/details", parameters: parameters, completion: {(success,data,error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - parameters:
        ...
        - accessType: enumeration of types of access ( read, delete, all, update )
     */
    open func grantUserAccess(toFile id: String, toUser userid: String, accessType: docAccessTypes, completion: BaasCompletion) {
        assert(id != "" && userid != "", "ID of file and username cannot be empty")
        put("/file/"+id+"/"+accessType.rawValue+"/user/"+userid, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - parameters:
        ...
        - accessType: enumeration of types of access ( read, delete, all, update )
     */
    open func grantRoleAccess(toFile id: String, toRole roleid: String, accessType: docAccessTypes, completion: BaasCompletion) {
        assert(id != "" && roleid != "", "ID of file and role cannot be empty")
        put("/file/"+id+"/"+accessType.rawValue+"/role/"+roleid, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - parameters:
        ...
        - accessType: enumeration of types of access ( read, delete, all, update )
     */
    open func revokeUserAccess(toFile id: String, toUser userid: String, accessType: docAccessTypes, completion: BaasCompletion) {
        assert(id != "" && userid != "", "ID of file and username cannot be empty")
        delete("/file/"+id+"/"+accessType.rawValue+"/user/"+userid, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
    ...
    /**
     ...
     - parameters:
        ...
        - accessType: enumeration of types of access ( read, delete, all, update )
     */
    open func revokeRoleAccess(toFile id: String, toRole roleid: String, accessType: docAccessTypes, completion: BaasCompletion) {
        assert(id != "" && roleid != "", "ID of file and role cannot be empty")
        delete("/file/"+id+"/"+accessType.rawValue+"/role/"+roleid, parameters: [:], completion: {(success, data, error) in
            completion(success, data, error)
        })
    }
     ...
     */
    open func getAsset(_ name: String, completion: BaasFileCompletion) {
        ...
        getFile("/asset/"+name, completion: {(success, data, error) in
            completion(success,data,error)
        })
    }
     ...
     */
    open func askToEnablePushNotifications() {
        if #available(iOS 10.0, *){
     ...
     */
    open func enablePushNotifications(_ deviceToken: Data, completion: BaasCompletion) {
        let token = deviceToken.reduce("", {$0 + String(format: "%02X", $1)})
        self.user.pushToken = token
        if !self.user.pushEnabled {
            put("/push/enable/ios/"+token, parameters: [:], completion: {(success, data, error) in
                if success {
    ...
    fileprivate func getHeaders(_ contType: String = "application/json") -> [String: String] {
        var h = [String:String]()
     ...
     */
    open func get(_ url: String, parameters:[String:AnyHashable], completion: BaasCompletion) {
        check()
                ...
                if let data = data {
                    ...
                    if sc >= 400 {
                        if self.config.retrylogin && sc == 401 && !self.isRetry {
                            self.retryRequest(request, error: NSError.init(domain: self.domain, code: sc, userInfo: ["message":json?["message"] as! String]), completion: { (success, data, error) in
                                completion(success,data,error)
                            })
                        } else {
    ...
    /**
     ...
     - parameters:
        ...
        - parameters: list of parameters for the query
     */
    open func post(_ url: String, parameters:[String:Any], completion: BaasCompletion) {
        check()
    ...
    /**
     ...
     - parameters:
        ...
        - parameters: list of parameters for the query
     */
    open func put(_ url: String, parameters:[String:Any], completion: BaasCompletion) {
        check()
        let requestUrl = self.base_url!+url
        let request = NSMutableURLRequest(url: URL(string: requestUrl)!, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: self.config.timeout)
    ...
    /**
     ...
     - parameters:
        ...
        - parameters: list of parameters for the query
     */
    open func delete(_ url: String, parameters:[String:Any], completion: BaasCompletion) {
        check()
        let requestUrl = self.base_url!+url
        let request = NSMutableURLRequest(url: URL(string: requestUrl)!, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: self.config.timeout)
    ...
    fileprivate func retryRequest(_ request: NSMutableURLRequest, isPostImage:Bool = false, error:Error, completion: BaasCompletion) {
        ...
        DispatchQueue(label: "retry").async {
            self.retryLogin(pass, completion: {(success) in
                if !success {
                    self.config.logoutStrategy.notAuthorized()
                    completion(false, nil, error)
                } else {
                    let session = URLSession.shared
                    request.allHTTPHeaderFields = !isPostImage ? self.getHeaders() : self.getHeaders("multipart/form-data; boundary="+self.boundary)
                    let dataTask = session.dataTask(with: request as URLRequest, completionHandler: { (data, response, error) -> Void in
                        if (error != nil) {
                            completion(false, nil, error)
                        } else {
    ...
    fileprivate var isRetry = false
    fileprivate func retryLogin(_ pass: String, completion: @escaping (Bool) -> Void) {
        self.isRetry = true
        if !BaasBox.shared.user.isSocialLogin {
            self.config.loginStrategy.login(self.user.username, password: pass, completion: {(success, data, error) in
                self.isRetry = false
                completion(success)
            })
        } else {
            self.config.loginStrategy.socialLogin(getSocialType(self.user.socialType), token: pass, completion: { (success, data, error) in
                self.isRetry = false
                completion(success)
            })
        }
    }
    fileprivate func getSocialType(_ s: String) -> socialTypes {
        switch s {
        case "facebook":
            return .facebook
        case "google":
    ...
    fileprivate func postImage(_ url: String, image:Data, attachedData: [String: AnyHashable] = [:], permissions: [String: AnyHashable] = [:], completion: BaasCompletion) {
        check()
        let requestUrl = self.base_url!+url
        let request = NSMutableURLRequest(url: URL(string: requestUrl)!, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: self.config.timeout)
        request.httpMethod = "POST"
        request.allHTTPHeaderFields = getHeaders("multipart/form-data; boundary="+boundary)
        request.httpBody = createImageBody(image, attachedData: attachedData, permissions: permissions)
        let session = URLSession.shared
        let dataTask = session.dataTask(with: request as URLRequest, completionHandler: { (data, response, error) -> Void in
            if (error != nil) {
                completion(false, nil, error)
                ...
                if let data = data {
                    ...
                    if sc >= 400 {
                        if self.config.retrylogin && sc == 401 {
                            self.retryRequest(request, isPostImage: true, error: NSError.init(domain: self.domain, code: sc, userInfo: ["message":json?["message"] as! String]), completion: { (success, data, error) in
                                completion(success,data,error)
                            })
                        } else {
                            if sc == 401 && !self.config.retrylogin { self.config.logoutStrategy.notAuthorized() }
    ...
    fileprivate func createImageBody(_ image: Data, attachedData: [String:AnyHashable] = [:], permissions: [String:AnyHashable] = [:]) -> Data {
        let body = NSMutableData()
        let mimetype = "image/jpeg"
        body.append("--\(boundary)\r\n".data(using: String.Encoding.utf8)!)
        body.append("Content-Disposition:form-data; name=\"file\"; filename=\"\(UUID().uuidString)\"\r\n".data(using: String.Encoding.utf8)!)
    ...
    fileprivate func getFile(_ url: String, completion: BaasFileCompletion) {
    ...
    fileprivate func postNotif(_ url: String, parameters:[String:Any], completion: BaasCompletion) {
        check()
        let requestUrl = self.base_url!+url
        let request = NSMutableURLRequest(url: URL(string: requestUrl)!, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: self.config.timeout)
        request.httpMethod = "POST"
    ...
    fileprivate let domain = "com.baasbox.sdk"
    fileprivate let boundary = "BAASBOX_BOUNDARY_STRING"
